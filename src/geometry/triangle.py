...
class Triangle(Primitive):
    def __init__(self, center, material, p1, p2, p3, max_ray_depth, shadow=True):
        ...
        self.collider_list += [
            Triangle_Collider(assigned_primitive=self, p1=p1, p2=p2, p3=p3)
        ]
...
class Triangle_Collider(Collider):
    def __init__(self, assigned_surface, p1, p2, p3):
        ...
        ) / 3  # one possible definition of center. Used for intersect().
    ...
    def intersect(self, O, D):
        ...
        M_C = M - self.centroid
        hit_inside = (
            (self.n31.dot(M - self.p1) >= 0)
            & (self.n12.dot(M - self.p2) >= 0)
            & (self.n23.dot(M - self.p3) >= 0)
            & (NdotC_O * NdotD > 0)
        )
        hit_UPWARDS = NdotD < 0
