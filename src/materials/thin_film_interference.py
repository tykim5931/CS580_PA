...
class ThinFilmInterference(Material):
    def __init__(self, thickness, noise=0.0, **kwargs):
        super().__init__(**kwargs)
        self.thickness = thickness

        # precomputed reflectance vs cosθI (vertical axis) and thickness (horizontal axis)
        self.thin_film_interference_reflectance = load_image(
            "src/textures/thin_film_interference_n=1.4.png"
        )
        self.thickness_noise = load_image("src/textures/noise.png")
    ...
    def get_color(self, scene, ray, hit):
        ...
        if ray.depth < hit.surface.max_ray_depth:
            ...
            if self.noise_factor != 0.0:
                thickness += self.noise_factor * (
                    self.thickness_noise[
                        -(
                            (v * self.thickness_noise.shape[0] * 0.5).astype(int)
                            % self.thickness_noise.shape[0]
                        ),
                        (u * self.thickness_noise.shape[1] * 0.5).astype(int)
                        % self.thickness_noise.shape[1],
                    ].T
                    - 0.5
                )
                Fim = self.thin_film_interference_reflectance[
                    (cosθi * self.thin_film_interference_reflectance.shape[0]).astype(
                        int
                    ),
                    thickness.astype(int),
                ]
            else:
                Fim = self.thin_film_interference_reflectance[
                    (cosθi * self.thin_film_interference_reflectance.shape[0]).astype(
            ...
            nudged = hit.point - N * 0.000001  # nudged for transmitted ray
            T = 1.0 - F
            transmitted_color = (
                get_raycolor(
                    Ray(
